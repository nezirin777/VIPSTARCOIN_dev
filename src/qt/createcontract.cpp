...
void CreateContract::setModel(WalletModel *_model)
{
    ...
    if (bCreateUnsigned) {
        ...
        ui->pushButtonCreateContract->setToolTip(tr("Creates a Partially Signed VIPSTARCOIN Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    }
}
...
void CreateContract::on_createContractClicked()
{
    if(isDataValid())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("This will produce a Partially Signed VIPSTARCOIN Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
