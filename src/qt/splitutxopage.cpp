    ...
    m_mode(mode)
{
    ...
    ui->labelAddress->setToolTip(tr("The vipstarcoin address to split utxos."));
...
void SplitUTXOPage::setModel(WalletModel *_model)
{
    ...
    // update the display unit, to not use the default ("VIPS")
    updateDisplayUnit();
    ...
    if (bCreateUnsigned) {
        ...
        ui->splitCoinsButton->setToolTip(tr("Creates a Partially Signed VIPSTARCOIN Transaction (PSBT) for use with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
    }
...
void SplitUTXOPage::on_splitCoinsClicked()
{
    if(m_model && m_model->getOptionsModel())
    {
        ...
        if (bCreateUnsigned) {
            ...
            questionString.append(tr("Please, review your transaction proposal. This will produce a Partially Signed VIPSTARCOIN Transaction (PSBT) which you can copy and then sign with e.g. an offline %1 wallet, or a PSBT-compatible hardware wallet.").arg(CLIENT_NAME));
